...
The RPM spec file provided here is for manna-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://manna.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[manna git repository](https://github.com/manna/manna) but are not part of
...
Currently only `Source10` is used, for the example `manna.conf` file.
...
in the manna git repository. At present only an SVG version of the manna
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the manna
...
needed when 0.12.1 is released, a proper fix is already in the manna git
master branch.
...
idea to build manna Core against a static version of that library compiled
...
The `manna.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `manna-server` package. At this point, most RPM based
...
The `manna-server` package should have SELinux support. How to properly do
...
manna-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/manna/). He has been
packaging manna for Fedora at least since 2012.
...
4. I renamed the `manna` package that contains the Qt GUI to `manna-core` as
that appears to be how the general population refers to it, in contrast to
`manna-xt` or `manna-classic`. I wanted to make sure the general population
