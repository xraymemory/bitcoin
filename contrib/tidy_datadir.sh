#!/bin/bash
# Copyright (c) 2013 The manna Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete manna database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no manna datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old manna datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected manna 0.7 datadir."
    ;;
  3)
    echo "Detected manna pre-0.8 datadir."
    ;;
  4)
    echo "Detected manna 0.8 datadir."
    ;;
esac
