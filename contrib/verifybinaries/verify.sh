...
###   This script attempts to download the signature file SHA256SUMS.asc from manna.org
...
WORKINGDIR="/tmp/manna_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://manna.org/bin/"
VERSIONPREFIX="manna-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'manna-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the manna Core binary release signing key installed?"
   fi
